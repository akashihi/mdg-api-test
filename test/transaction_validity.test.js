...
it('Multi currency transaction without rate are not allowed', async () => {
    ...
    //No way to remove field from the template
    await pactum.spec('expect error', {status_code: 412, error_code: 'TRANSACTION_AMBIGUOUS_RATE'})
        .post('/transaction')
        .withJson( {
            "data": {
                "attributes": {
...
it('Multi currency transaction with rate set to all operations are not allowed', async () => {
    ...
    //No way to remove field from the template
    await pactum.spec('expect error', {status_code: 412, error_code: 'TRANSACTION_NO_DEFAULT_RATE'})
        .post('/transaction')
        .withJson( {
...
it('Multi currency transaction with 0 rate is not allowed', async () => {
    ...
    //No way to remove field from the template
    await pactum.spec('expect error', {status_code: 412, error_code: 'TRANSACTION_ZERO_RATE'})
        .post('/transaction')
        .withJson( {
            '@DATA:TEMPLATE@': 'Transaction:MultiCurrency',
            '@OVERRIDES@': {
